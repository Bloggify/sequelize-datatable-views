...
export default () => {
    if (!Bloggify.partials.datatable_view) {
        Bloggify.partials.datatable_view = `${__dirname}/views/datatable_view.ejs`;
    }
};
 ...
 * @returns {Function} The action to draw the table.
 */
export const tableDraw = (headers, models, options) => {
    return async ctx => {
        ctx.query.order = ctx.query.order || [];
